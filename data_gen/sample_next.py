...
def sample_name(edge_case=None):
    ...
    if edge_case == "hyphenated_surname":
        ...
        # non_slavic_name и остальные — обычный сэмпл из faker
        # (faker содержит нестандартные имена: Август, Адриан, Ян и т.д.)
        if gender == "M":
...
def print_stats():
    print("=== Целевые числа датасета (9 000 примеров) ===\n")
    total = 9000

    print("По типу примера:")
    for k, v in ENTITY_TYPE_PROBS.items():
        print(f"  {k:<16} {v*100:5.1f}%  ≈ {round(v*total):>5}")

    print("\nПо домену (целевые кол-ва):")
    targets = {
        "DELIVERY": 800, "DIALOG": 1000, "AUTO": 600, "BANK": 600,
        "HR": 600, "RE": 500, "TELECOM": 600, "NAME": 500, "ADDRESS": 500,
        "MEDICAL": 450, "GOV": 550, "LEGAL": 550, "SOCIAL": 500,
        "NEGATIVE (neg_synth)": 2250,
    }
    for domain, t in targets.items():
        print(f"  {domain:<25} ~{t}")

    print("\nФормы имени:")
    for k, v in NAME_FORM_PROBS.items():
        print(f"  {k:<16} {v*100:5.1f}%")

    print("\nФормы адреса:")
    for k, v in ADDR_FORM_PROBS.items():
        print(f"  {k:<14} {v*100:5.1f}%")

    print("\nФлаги:")
    print("  has_typo      5%  (позитивные)")
    print("  multi_entity  12% (NAME_ONLY / ADDRESS_ONLY / BOTH)")
    print("  edge_case     20% (позитивные)")
    print("\nПозиция сущности:")
    print("  beginning     30%  (45% → см. multi)")
    print("  middle        30%  (20% при multi_entity)")
    print("  end           25%  (17% при multi_entity)")
    print("  mixed         15%  (45% при multi_entity)")
    print("  null               (NEGATIVE)")
    print("\nСтиль:  formal 35%  informal 35%  mixed 30%")
    print("Длина:  short  20%  medium   55%  long  25%")
    print("\nЭмоциональный тон:")
    for k, v in EXPRESSION_PROBS.items():
        print(f"  {k:<12} {v*100:5.1f}%  ≈ {round(v*total):>5}")
    print("\nИмена: Faker('ru_RU')")
    print("  Фамилии М/Ж: 250/250 | Имена М/Ж: 321/80 | Отчества М/Ж: 133/80")
    print("Адреса: Faker('ru_RU')")
    print("  Города: 649 | Улицы: 297 | Типы улиц: 7")
